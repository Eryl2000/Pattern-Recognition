...
class Eigenface
{
    ...
    private:
        ...
        MatrixXf ComputeEigenSpaceValues(MatrixXf images) const;

        // images - (N^2 x M, K, 1)-matrix
        // Subtracts the average face from each column of images and returns the resulting value
        MatrixXf NormalizeImages(MatrixXf images) const;
        ...
        MatrixXf ReconstructImages(MatrixXf eigenspaceValues, int eigenCount) const;
        ...
        // eigenCount - number of eigenvectors considered in the calculation
            // Must be less than or equal to M
        float MahalanobisDistance(VectorXf eigenspaceImage1, VectorXf eigenspaceImage2, int eigenCount) const;

        // Returns the EuclideanDistance between the eigenspace representations of the two images
        // eigenspaceImage - (M)-vector
        // eigenCount - number of eigenvectors considered in the calculation
            // Must be less than or equal to M
        float EuclideanDistance(VectorXf eigenspaceImage1, VectorXf eigenspaceImage2, int eigenCount) const;
};
